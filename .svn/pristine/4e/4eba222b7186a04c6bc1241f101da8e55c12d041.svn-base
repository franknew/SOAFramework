namespace AustinHarris.JsonRpc
{
    ...
    public class Handler
    {
        ...
        private Handler(string sessionId)
        {
            ...
            this.Handlers = new ConcurrentDictionary<string, Delegate>();
        }
        ...
        public JsonResponse Handle(JsonRequest Rpc, Object RpcContext = null, Action<JsonResponse> callback = null)
        {
            ...
            else if (Rpc.Params is IDictionary)
            {
                ...
                for (int i = 0; i < metadata.parameters.Length; i++)
                {
                    ...
                    {
                        if (dic[metadata.parameters[i].Name] is JObject) parameters[i] = CleanUpParameter(dic[metadata.parameters[i].Name], metadata.parameters[i]);
                        else parameters[i] = dic[metadata.parameters[i].Name].ConvertTo(metadata.parameters[i].ObjectType);
                    }
                }
            }
        ...
        private object CleanUpParameter(object p, SMDAdditionalParameters metaData)
        {
            ...
            if (bob != null)
            {
                ...
                try
                {
                    return Convert.ChangeType(bob, metaData.ObjectType);
                }
                catch (Exception ex)
                { }
