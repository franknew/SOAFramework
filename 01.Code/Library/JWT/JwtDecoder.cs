...
namespace JWT
{
    ...
    public sealed class JwtDecoder : IJwtDecoder
    {
        ...
        /// <exception cref="ArgumentOutOfRangeException" />
        /// <exception cref="FormatException" />
        public string Decode(string token, IList<byte[]> keys, bool verify)
        {
            if (String.IsNullOrWhiteSpace(token))
                throw new ArgumentException(nameof(token));
        ...
        /// <exception cref="ArgumentOutOfRangeException" />
        /// <exception cref="FormatException" />
        public IDictionary<string, object> DecodeToObject(string token, IList<byte[]> keys, bool verify) => DecodeToObject<Dictionary<string, object>>(token, keys, verify);
        ...
        /// <exception cref="ArgumentOutOfRangeException" />
        /// <exception cref="FormatException" />
        public T DecodeToObject<T>(string token, IList<byte[]> keys, bool verify)
        {
        ...
        /// <exception cref="ArgumentOutOfRangeException" />
        /// <exception cref="FormatException" />
        public void Validate(JwtParts jwt, IList<byte[]> keys)
        {
            if (jwt == null)
                throw new ArgumentNullException(nameof(jwt));
            if (keys == null || keys.Count == 0)
        ...
        private static IList<byte[]> GetBytes(IEnumerable<string> input)
        {
            return input.Select(key => GetBytes(key)).ToList();
        }

        private static bool DoesKeysHaveValues(IList<byte[]> keys)
        {
