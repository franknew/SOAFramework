...
namespace SOAFramework.WebServiceCore
{
    ...
    public class WSPCore
    {
        ...
        public string CallMethod(string RequestString)
        {
            ...
            ServerResponse srResponse = new ServerResponse();
            ...
            try
            {
                ...
                {
                    srResponse.ErrorMessage = "解析XML错误，请检查XML格式是否正确！";
                    srResponse.CallSucceded = false;
                }
                try
                {
                    //通过XML解析出类，方法和参数
                    ...
                    if (crRequest.RequestData.MethodArgs != null && crRequest.RequestData.MethodArgs.Length > 0)
                    {
                        ...
                        {
                            objArgs[i] = crRequest.RequestData.MethodArgs[i].ArgValue;
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    srResponse.ErrorMessage = tempEx.Message;
                    srResponse.CallSucceded = false;
                    srResponse.StackTrace = tempEx.StackTrace;
                }
                finally
                {
                    srResponse.ResponseType = WSDataType.JSON;
