...
namespace AustinHarris.JsonRpc
{
    public static class JsonRpcProcessor
    {
        ...
        {
            if (string.IsNullOrEmpty(async.JsonRpc)) async.JsonRpc = Newtonsoft.Json.JsonConvert.SerializeObject(async.Request);
        ...
        public static Task<string> Process(string sessionId, JsonRequest request, object context = null)
        {
            return Task<string>.Factory.StartNew((_) =>
            {
                var tuple = (Tuple<string, JsonRequest, object>)_;
                return ProcessInternal(tuple.Item1, tuple.Item2, tuple.Item3);
            }, new Tuple<string, JsonRequest, object>(sessionId, request, context));
        }

        public static string ProcessSync(string sessionId, JsonRequest request, object context = null)
        {
            return ProcessInternal(sessionId, JsonHelper.Serialize(request), context);
        }
        ...
        public static List<JsonResponse> ProcessReturnResponse(string sessionId, string jsonRpc, object jsonRpcContext = null)
        {
        ...
        public static List<JsonResponse> ProcessReturnResponse(string sessionId, JsonRequest request, object jsonRpcContext = null)
        {
            ...
            catch (Exception ex)
            {
                responselist.Add(new JsonResponse
                {
                    Error = handler.ProcessParseException("", new JsonRpcException(-32700, "Parse error", ex)),
                    Success = false,
                });
            }
            return responselist;
        ...
        private static string ProcessInternal(string sessionId, JsonRequest request, object jsonRpcContext)
        {
            return ProcessInternal(sessionId, new List<JsonRequest> { request }, jsonRpcContext);
        }
